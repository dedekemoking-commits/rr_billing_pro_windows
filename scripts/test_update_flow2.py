#!/usr/bin/env python3
"""Test script v2: generate RSA keys, fake asset, manifest, sign, then load scripts/check_update.py and call check_for_update with file:// URLs."""
...
# Dynamically load check_update module
check_path = os.path.join(ROOT, 'check_update.py')
spec = importlib.util.spec_from_file_location('check_update', check_path)
check = importlib.util.module_from_spec(spec)
spec.loader.exec_module(check)

try:
    res = check.check_for_update('file://' + manifest_path.replace('\\', '/'), pub_path, '2.1', None)
