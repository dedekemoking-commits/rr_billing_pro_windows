...
class LicenseManager:
    ...
    def get_status(current_user: str = "") -> dict:
        ...
        # ── Cloud-synced (Firebase) license — skip offline verification ──────
        if lic.get("firebase_sync") and lic.get("aktif"):
            expiry = lic.get("expiry", "")
            sisa_hari = 0
            if expiry:
                try:
                    sisa_hari = max(0, (datetime.fromisoformat(expiry).date() - date.today()).days)
                except Exception:
                    pass
            if sisa_hari > 0:
